...
public class Batallon {
    ...
    public boolean asignarSoldadoMision(String identificadorSoldado, String idMision) {
        ...
        for (Soldado soldado : listSoldados) {
            if (soldado.getIdentificador().equals(identificadorSoldado)
                    && soldado.getEstadoSoldado().equals(EstadoSoldado.DISPONIBLE)) {
                newSoldado = soldado;
            }
        }
        if (newMision != null && newSoldado != null) {
            newMision.getPersonal().add(newSoldado);
            newSoldado.setEstadoSoldado(EstadoSoldado.NO_DISPONIBLE);
            asignar = true;
        }
        return asignar;
    }
